...
public class FluidInfuserContainerMenu extends WootContainerMenu {
    private final Level level;
    public FluidInfuserBlockEntity blockEntity;

    public FluidInfuserContainerMenu(int id, Level level, BlockPos pos, Inventory playerInventory, Player player) {
        super(BlocksRegistry.FLUID_INFUSER_BLOCK_MENU.get(), id);
        this.level = level;
        BlockEntity blockEntity = level.getBlockEntity(pos);
        if(!(blockEntity instanceof FluidInfuserBlockEntity be)) return;
        this.blockEntity = be;
        this.data = be.data;
    ...
    public FluidInfuserContainerMenu(int windowId, Inventory inv, FriendlyByteBuf data) {
    ...
    public @NotNull ItemStack quickMoveStack(@NotNull Player player, int index) {
        ...
        if (index >= MACHINE_INV_SIZE) {
            // player slot
            if (FluidInfuserRecipe.Validator.isCatalystValid(stack)) {
                // -> machine
                startIndex = 0;
