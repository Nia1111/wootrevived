...
public class ItemInfuserContainerMenu extends WootContainerMenu {
    ...
    public ItemInfuserContainerMenu(int id, Level level, BlockPos pos, Inventory playerInventory, Player player) {
        super(BlocksRegistry.ITEM_INFUSER_BLOCK_MENU.get(), id);
        this.level = level;
        BlockEntity blockEntity = level.getBlockEntity(pos);
        if(!(blockEntity instanceof ItemInfuserBlockEntity be)) return;
        this.blockEntity = be;
        this.data = be.data;
    ...
    private void createMachineInputSlots(IItemHandler machineInventory){
        this.addSlot(new SlotItemHandler(machineInventory, 0, 65, 40));
        this.addSlot(new SlotItemHandler(machineInventory, 1, 85, 40));
        this.addSlot(new SlotItemHandler(machineInventory, 2, 141, 40){
            @Override
